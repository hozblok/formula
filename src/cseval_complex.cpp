...
const Complex cseval_complex<Complex>::ZERO = Complex("0", "0");
...
const std::map<std::string, Complex (*)(Complex)> cseval_complex<Complex>::functionsOneArg = {
    ...
    {std::string("tan"), cseval_complex<Complex>::_tan}, {std::string("atan"), cseval_complex<Complex>::_atan},
    {std::string("log"), cseval_complex<Complex>::_log}, {std::string("sqrt"), cseval_complex<Complex>::_sqrt},
    {std::string("exp"), cseval_complex<Complex>::_exp}};

template <typename Complex>
const std::map<std::string, Complex (*)(Complex, Complex)>
    cseval_complex<Complex>::functionsTwoArgsDLeft = {
        {std::string("+"), cseval_complex<Complex>::_one},       {std::string("-"), cseval_complex<Complex>::_one},
        {std::string("*"), cseval_complex<Complex>::_mul1},      {std::string("/"), cseval_complex<Complex>::_truediv1},
        {std::string("^"), cseval_complex<Complex>::_pow1},      {std::string("sin"), cseval_complex<Complex>::_sin_d},
        {std::string("asin"), cseval_complex<Complex>::_asin_d}, {std::string("cos"), cseval_complex<Complex>::_cos_d},
        {std::string("acos"), cseval_complex<Complex>::_acos_d}, {std::string("tan"), cseval_complex<Complex>::_tan_d},
        {std::string("atan"), cseval_complex<Complex>::_atan_d}, {std::string("log"), cseval_complex<Complex>::_log_d},
        {std::string("sqrt"), cseval_complex<Complex>::_sqrt_d}, {std::string("exp"), cseval_complex<Complex>::_exp_d}};

template <typename Complex>
const std::map<std::string, Complex (*)(Complex, Complex)>
    cseval_complex<Complex>::functionsTwoArgsDRight = {{std::string("+"), cseval_complex<Complex>::_one},
...
#ifdef CSDEBUG
  std::cout << "copy constructor cseval_complex+, kind: " << kind_ << ", id: " << id_
            << std::endl;
#endif
  if (other.left_eval_) {
...
template <typename Complex>
cseval_complex<Complex>::cseval_complex(std::string expression, char imaginary_unit)
    : kind_('e'),
      id_(""),
      value_("0"),
      left_eval_(nullptr),
...
#ifdef CSDEBUG
  std::cout << "constructor cseval_complex, expression:" << expression << std::endl;
#endif
  if (expression.empty()) {
...
is empty");
  ...
  for (std::string::const_iterator it = kOperations.cbegin();
    ...
    if (op_index != kNpos) {
      ...
      left_eval_ = new cseval_complex<Complex>(expression.substr(0, op_to_index.at(*it)));
      right_eval_ =
          new cseval_complex<Complex>(expression.substr(op_to_index.at(*it) + 1));
      return;
    }
  }
...
location and / or number of brackets");
  ...
  if (iLeftBracket == std::string::npos) {
    ...
    if (index_comma != std::string::npos) {
      ...
      right_eval_ = new cseval_complex<Complex>(expression.substr(index_comma + 1));
    } else {
...
Complex cseval_complex<Complex>::calculate(
    ...
    const std::map<std::string, Complex (*)(Complex, Complex)> &mapFunctionTwoArgsValue,
    const std::map<std::string, Complex (*)(Complex)> &mapFunctionOneArgValue) const {
  if (kind_ == 'n') {
    return value_;
  } else if (kind_ == 'v') {
...
calculation of the expression, variable name: '%s'") %
    ...
    if (left_eval_ && right_eval_) {
      ...
      typename std::map<std::string, Complex (*)(Complex, Complex)>::const_iterator
          itFunction;
      ...
      left = left_eval_->calculate(variables_to_values, mapFunctionTwoArgsValue,
                                   mapFunctionOneArgValue);
      typename std::map<std::string, Complex (*)(Complex)>::const_iterator itFunction;
...
Complex cseval_complex<Complex>::calculate_derivative(
    ...
    const std::map<std::string, Complex (*)(Complex, Complex)> &mapFunctionDerivLeft,
    const std::map<std::string, Complex (*)(Complex, Complex)> &mapFunctionDerivRight)
    const {
  if (kind_ == 'n') {
    return ZERO;
  } else if (kind_ == 'v') {
...
template <typename Complex>
std::unordered_map<char, size_t> cseval_complex<Complex>::operations_outside_parentheses(
